...
export default function AddField({ onAdded }) {
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    if (!name || !lat || !lon) {
      return alert("Please provide name and coordinates");
    }
    setLoading(true);
    try {
      const payload = {
  ...
  return (
    <div className="max-w-lg mx-auto">
      <form onSubmit={handleAdd} className="card p-4 space-y-3">
        ...
        <input className="border p-2 rounded w-full" placeholder="Field name" value={name} onChange={(e)=>setName(e.target.value)} />
        <div className="grid grid-cols-2 gap-2">
          <input className="border p-2 rounded" placeholder="Latitude" value={lat} onChange={(e)=>setLat(e.target.value)} />
          <input className="border p-2 rounded" placeholder="Longitude" value={lon} onChange={(e)=>setLon(e.target.value)} />
        </div>
        <div>
          <label className="text-sm text-gray-600">Size (km)</label>
          <input type="number" step="0.1" className="border p-2 rounded w-32" value={sizeKm} onChange={(e)=>setSizeKm(e.target.value)} />
        </div>
        <div>
          <button className="bg-brand text-white px-4 py-2 rounded">{loading ? "..." : "Add Field"}</button>
        </div>
      </form>
    </div>
  );
